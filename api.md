...
Conventions the model relies on, beyond the exact result strings: n2 performs best when the full five-tool set is served — if you must run a reduced dated set, keep at least `computer_batch` and `bash`, because the model does shell work through the `bash` tool and driving a terminal through the GUI instead is far less efficient; and a `read` of an image file shows the model the image itself. Integrating your own infrastructure is therefore three pieces — the GUI primitives over your capture/input APIs, `run_bash_command` over its shell, and the file tools. Start from the example whose shape matches where your desktop lives:

- **Your code runs on (or inside) the machine it drives** — a local desktop, or an agent process you install in your own VMs: start from [`examples/navigator_n2/linux_adapter.py`](examples/navigator_n2/linux_adapter.py), which drives the display in-process (pyautogui, mss, local subprocesses) with nothing between the handler and the machine.
- **Your desktops sit behind a remote API** — a sandbox provider, or your own fleet service exposing mouse/keyboard/screenshot/shell calls: start from [`examples/navigator_n2/cua_adapter.py`](examples/navigator_n2/cua_adapter.py), the full-surface remote adapter. Everything vendor-specific is confined to its `self.sandbox.*` calls — swap those for your API's and keep the structure: converting units at the API boundary, `bash` with a persistent working directory over a remote shell (including the timeout and background-run forms), and the file tools via the mixin. [`examples/navigator_n2_daytona.py`](examples/navigator_n2_daytona.py) is a second, more compact example of the same remote shape over REST primitives.
- **macOS**: `yutori.navigator.macos.MacOSComputer` is a shipped runtime, not an example — use it directly.

Whichever shape you start from, the tool contracts are identical; only the transport differs.
