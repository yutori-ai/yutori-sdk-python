...
`MacOSComputer` is the native macOS adapter — CuaDriver session, capture/input, shell lifecycle, cancellation, recovery, and the optional presentation overlay. It is what Yutori MCP runs; local shell execution stays off unless the caller enables it. With `presentation=True` a foreground run paints the full-screen overlay (cursor capsule, click pulses, and the shell rail under the menu bar) and offers the activity window — a floating, non-activating panel carrying the conversation with the model: the task, its thinking, every action, and every shell command, opened from *Show activity* in the menu bar item's menu. The window lets the mouse through: a click on its body reaches whatever is behind it, so neither the operator's work nor a model click posted to the desktop is swallowed by it; only the grip strip along its top takes the mouse (drag it to move the window, or use its close button), and model input on the grip is refused the way input on the Stop item is (`MacOSActionRefusedError`). The window does not depend on how the run drives the Mac (window scope shows the same one, with the driven window's live frame above it), and the model never sees it: every panel Yutori draws opts out of screen capture (`NSWindow.sharingType = .none`), which the adapter verifies once at start by capturing a frame with a small probe panel on screen (`presentation.capture_exclusion == "excluded"`, telemetry event `capture_exclusion`). On a macOS where the probe shows up in the frame, the overlay and the window are hidden around every capture instead, as before (`"leaked"`). That opt-out also keeps the panels out of screen recordings, screen shares, and VNC; `exclude_overlay_from_capture=False` keeps them capturable and moves the exclusion to the capturer instead: the model's desktop frames then come from the overlay host's own ScreenCaptureKit capture of the same display, with every window of the host process filtered out (`presentation.capture_source == "overlay"`, verified with the same probe). Should that capture ever fail, the frames come from the driver again with the overlay hidden around each (`capture_source == "driver"`, telemetry event `capture_source`).
...
Window scope drives one application window in the background instead of the visible desktop: `MacOSComputer(scope="window", allow_foreground_fallback=False)`, then `await computer.set_window_target(MacOSWindowTarget(pid, window_id))` (or pass `target_window=`); `resolve_window_target(pid)` picks a window from `list_windows` via the pure `select_target_window` helper. `launch_app` starts apps hidden, and the driver refuses raw keyboard input to a hidden or minimized window (`minimized_or_hidden_window`), so call `await computer.unhide_app(pid)` first: it shows the app's windows behind the user's without activating it (AppKit `unhide` through the `osascript` ObjC bridge, no Apple Events grant). From then on `screenshot()` returns only that window (`get_window_state`; `native_width`/`native_height` are the window capture) and the model's coordinates are window-local, while every input carries `target: {kind: "window", pid, window_id}` with `delivery_mode: "background"`, so the user's focus, pointer, and frontmost app are never taken. No desktop frame is captured at any point. With `presentation=True` the full-screen overlay is replaced by three surfaces the user can watch while they keep working: a menu bar status item (the Yutori mark) whose menu shows the run title, the latest step, the latest frame of the driven window, a Show activity toggle, and Stop (also ⇧⌘Esc); the same click-through shell rail under the menu bar a foreground run paints, so a command running on this Mac is visible without opening anything; and the same activity window a foreground run offers, here with the live frame of the driven window above the conversation. Two additional constructor parameters control which surfaces appear: `show_status_item=False` suppresses the menu bar item, its hotkey, and the activity window, leaving only the shell rail (use this when the embedding host owns status, activity, Stop, and hotkey itself); `background_focus_overlay=True` (requires `scope="window"`) shows the Navigator pointer and reasoning overlay — cursor capsule, click pulses, and reasoning text — only while the target window's owner application is the frontmost app, so the overlay appears and disappears as the user switches between the driven app and their own work. When `background_focus_overlay` is set, `presentation.set_window_target(target)` updates the overlay's watched pid/window without affecting capture; the adapter calls it automatically on target changes, and `set_window_target(None)` clears the overlay. While the menu is open or the activity window is shown, the adapter streams the window at about two frames per second over a second driver connection, so the model's own captures and actions never wait behind it (`preview_frames_sent`); nothing streams when nobody is looking (`presentation_status.state == "active"`, cursor `"hidden"`); with `presentation=False` nothing is shown (`degradation_reason == "window_mode"`). The focus guard is skipped, `move` is recorded but never posted, and modified clicks are refused unless fallback is allowed. Each action's driver envelope is parsed: an `effect` of `suspected_noop`/`refused` or `escalation.recommended == "foreground"` means it did not land (`unverifiable` and `partial` count as landed), and an up-front refusal with `same_pid_keyboard_ambiguity` (the app has several windows, so process-scoped keys cannot be proven to reach this one) or `minimized_or_hidden_window` is treated the same way. A `same_pid_keyboard_ambiguity` refusal of `type_text` is first retried through the driver's accessibility rung, which stays open because an `AXSelectedText` write is bound to one element and no sibling window can receive it: the adapter re-reads the window's elements (`get_window_state`, no screenshot, `max_elements` bounded) and re-sends the text addressed to the smallest text field containing the point the model last clicked, or to the window's only text field (`delivery_counts["accessibility_rungs"]`). Native controls take the write (a Cocoa text view, a browser's own address bar); web page content does not, so a browser tab still needs the foreground rung. `type()` sends every newline in its text as a Return keystroke (`press_key`), because the driver would otherwise insert the character literally through the accessibility write or send it as keycode 0: `"query\n"` submits the way it does on the other computers, the clicked field stays the typing target for the text after it, and a Return the driver refuses raises instead of being swallowed. A strict session raises `MacOSBackgroundDeliveryError` (recoverable, fresh frame, `.outcome`); with `allow_foreground_fallback=True` that one action is retried once with `delivery_mode: "foreground"`, where the driver fronts the window briefly and restores the prior app; the retry is skipped (`delivery_counts["fallback_skips"]`) when the window already changed after the background attempt, because the driver's verdict can be wrong for part of a keystroke sequence and re-sending would double-type. When the driver reports `window_id_not_found` or `window_owner_pid_mismatch`, the adapter rebinds to another window of the app, or runs `recover_target` if the process died: captures continue transparently, inputs raise `MacOSTargetWindowChangedError` with the new window's frame. A window that WindowServer still lists but no accessibility window backs (a closed dialog lingering as a zombie record, or a window on another Space: captures come back `degraded_reason: ax_window_unresolved`, inputs are refused with `off_space_or_ax_unresolved`) is left the same way when the app has another window; otherwise the frame is kept and inputs raise `MacOSBackgroundDeliveryError`. Telemetry: `last_action_outcome` / `action_outcomes` (`MacOSActionOutcome`), `delivery_counts` (`background_attempts`, `foreground_escalations`, `fallback_skips`, `accessibility_rungs`, `background_refusals`, `window_rebinds`), and `window_target_info`. Driver limits to expect: minimized windows drop keyboard commits, off-Space SwiftUI windows lose their accessibility tree (pixels still work), Chromium drops background right-clicks, and raw-input canvases ignore background input.
...
| `run_custom_tool(name, arguments) -> str` | Dispatch for a call to one of the caller's own `tools=` definitions (see [Changing the n2 tool set](#changing-the-n2-tool-set)); its returned text is the result, carried with the post-action frame | A recoverable `[ERROR] <name> was declared in tools= but this computer environment implements no run_custom_tool.` |
