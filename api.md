...
`N2InlineCompactor` — the loop's default (`compactor="auto"`) — implements a usage-triggered, tail-retaining policy: it preserves the initial user request, replaces older turns with a model-written working checkpoint, and keeps recent complete turns verbatim. The defaults reproduce n2's trained regime: long-horizon runs are trained at a 64K-token context with this same compaction prompt, triggered at 53,760 tokens (64,000 minus 10,240 of headroom). Keep the default compactor for best performance — `compactor=None` runs the model past its trained context toward the 128k serving limit, and a custom policy departs from the compaction prompt the model is trained with. To customize the trigger or retention, construct it explicitly:
