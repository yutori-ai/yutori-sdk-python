...
Conventions the model relies on, beyond the exact result strings: n2 performs best when the full five-tool set is served — if you must run a reduced dated set, keep at least `computer_batch` and `bash`, because the model does shell work through the `bash` tool and driving a terminal through the GUI instead is far less efficient; and a `read` of an image file shows the model the image itself. Integrating your own infrastructure is therefore three pieces — the GUI primitives over your capture/input APIs, `run_bash_command` over its shell, and the file tools. [`examples/navigator_n2/linux_adapter.py`](examples/navigator_n2/linux_adapter.py) is the vendor-free full-surface reference — nothing between the handler and the machine — and [`examples/navigator_n2/cua_adapter.py`](examples/navigator_n2/cua_adapter.py) adapts the same surface (including `bash`'s timeout and background-run forms) to a hosted sandbox API; [`examples/navigator_n2_daytona.py`](examples/navigator_n2_daytona.py) is a compact adapter over REST primitives.
