...
A computer-use agent using third-party [Daytona](https://www.daytona.io) infrastructure. `N2ComputerAgent` runs the loop, while this Yutori-maintained example provides a compact `DaytonaComputer` adapter plus sandbox lifecycle wiring. It serves the full current tool set: GUI and `bash` natively, and the file tools (`read`/`write`/`edit`) via the SDK's `ShellFileToolsMixin` — [direct_x11_adapter.py](navigator_n2/direct_x11_adapter.py) shows the same contract over direct X11 access, [cua_adapter.py](navigator_n2/cua_adapter.py) shows a full-surface API-backed sandbox, and [api.md](../api.md)'s "Navigator n2 loop" section documents the contract. The ephemeral sandbox is deleted, with deletion confirmation requested, when the run ends.
