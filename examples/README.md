...
A computer-use agent using third-party [Daytona](https://www.daytona.io) infrastructure. `N2ComputerAgent` runs the loop, while this Yutori-maintained example provides a compact `DaytonaComputer` adapter plus sandbox lifecycle wiring. It serves the full current tool set: GUI and `bash` natively, and the file tools (`read`/`write`/`edit`) via the SDK's `ShellFileToolsMixin` — [linux_adapter.py](navigator_n2/linux_adapter.py) and [cua_adapter.py](navigator_n2/cua_adapter.py) show further wirings of the same mixin, and [api.md](../api.md)'s "Navigator n2 loop" section documents the contract. The ephemeral sandbox is deleted, with deletion confirmation requested, when the run ends.
