...
class Agent:
    def __init__(
        ...
        self._messages: list = []
        self._step_count = 0
        self._executor = PlaywrightActionExecutor()
    ...
    async def _take_screenshot(self) -> str:
        await prepare_page_for_model(self._page)
    ...
    def _url_suffix(self, current_url: str | None = None) -> str:
        return f"\n\nCurrent URL: {current_url or self._page.url}"
    ...
    async def _execute(self, tool_call: ChatCompletionMessageToolCall) -> str | None:
        ...
        try:
            result = await self._executor.execute_tool_call(context, tool_call)
        except PlaywrightActionError as exc:
            action_name = tool_call.function.name
            logger.error(f"Error executing {action_name}: {exc}")
            return f"[ERROR] Error executing {action_name}: {exc}{self._url_suffix()}"

        if isinstance(result, PlaywrightToolExecutionResult):
            body = result.output_text or f"Executed {result.trace}."
            return f"{body}{self._url_suffix(result.current_url)}"
        return f"{result}{self._url_suffix()}"
...
async def main():
    ...
    parser.add_argument(
        "--tool-set",
        default=default_config.tool_set,
    ...
    parser.add_argument(
        "--disable-tools",
        nargs="*",
        default=default_config.disable_tools,
